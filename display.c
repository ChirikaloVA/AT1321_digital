...
//	Display_checkForClearLine();
	while((sym = *pText++))
	{
		...
		{//перевод строки
			...
			{
				--lineIndex;
				display.text.gstrX = 0;	//это нужно для перевода строки, хотя y не меняется
			}
			//пересчет координаты х
			Display_justifyText(pText);
			bWordTested = FALSE;
...
//поймать лучь
//если задано 0xffff то поиск не ведется
...
void Display_warmup_display_start(void)
{
	...
	Display_outputTextByLang("IN PROGRESS\r\r\0""ERFULLUNG\r\r\0""IN PROGRESS\r\r\0""ВЫПОЛНЯЕТСЯ\r\r");	//"ВЫКЛ"
	Display_setTextColor(GREEN);	//set text color
	Display_setTextJustify(LEFT);
	Display_outputTextByLang("Please wait...\0""Bitte warten...\0""Please wait...\0""Пожалуйста ждите...");	//"ОТПУСТИТЕ КНОПКУ"
